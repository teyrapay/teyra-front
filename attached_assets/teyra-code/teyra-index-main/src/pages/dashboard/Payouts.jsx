...
export default function Payouts() {
  ...
  const total = payouts.filter(p => p.status === 'paid').reduce((s, p) => s + (p.amount || 0), 0);
  const inTransit = payouts.filter(p => p.status === 'in_transit').reduce((s, p) => s + (p.amount || 0), 0);
  const pending = payouts.filter(p => p.status === 'pending').reduce((s, p) => s + (p.amount || 0), 0);

  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div>
        <h1 className="font-syne text-2xl font-700 text-foreground">Payouts</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: 'Total Paid Out', value: `SAR ${total.toLocaleString()}`, icon: CheckCircle2, color: 'text-green-600', bg: 'bg-green-500/10' },
          { label: 'In Transit', value: `SAR ${inTransit.toLocaleString()}`, icon: ArrowDownToLine, color: 'text-blue-600', bg: 'bg-blue-500/10' },
          { label: 'Pending', value: `SAR ${pending.toLocaleString()}`, icon: Clock, color: 'text-yellow-600', bg: 'bg-yellow-500/10' },
        ].map(card => {
          ...
          return (
            <div key={card.label} className="bg-card border border-border rounded-2xl p-5">
              ...
              <div className="text-xl font-syne font-700 text-foreground">{card.value}</div>
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <h2 className="font-syne font-700 text-foreground">Payout History</h2>
            ...
            <tbody>
              ...
              {payouts.map(p => (
                <tr key={p.id} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-6 py-3.5 text-muted-foreground">{p.bank_name || '—'}</td>
